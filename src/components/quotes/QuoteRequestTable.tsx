            ...
            <TableRow key={request.id}>
              ...
              <TableCell>
                {request.formats && request.formats.length > 0 ? (
                  <Badge variant="secondary" className="hover:bg-secondary/80 cursor-pointer" onClick={() => viewDetails(request)}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p>{selectedRequest?.supplier_name}</p>
            </div>
            
            <div>
              <h3 className="text-sm font-medium text-muted-foreground">Status</h3>
              <p>{getStatusBadge(selectedRequest?.status || 'pending')}</p>
            </div>
            
            <div>
              <h3 className="text-sm font-medium text-muted-foreground">Requested Date</h3>
              <p>{selectedRequest?.requested_at ? format(new Date(selectedRequest.requested_at), "PPP") : 'N/A'}</p>
            </div>
            
            {selectedRequest?.expected_delivery_date && (
              <div>
                <h3 className="text-sm font-medium text-muted-foreground">Expected Delivery Date</h3>
                <p>{format(new Date(selectedRequest.expected_delivery_date), "PPP")}</p>
              </div>
            )}
