...
export function CostsAndSavingsTabsField() {
  ...
  useEffect(() => {
    if (
      ...
      if (
        !extraCostsFieldArray.fields ||
        extraCostsFieldArray.fields.length === 0
      ) {
  ...
  useEffect(() => {
    if (
      !defaultSavingsAdded &&
      currentOrganization?.default_savings &&
      currentOrganization.default_savings.length > 0
    ) {
      if (!savingsFieldArray.fields || savingsFieldArray.fields.length === 0) {
        const defaultSavings = currentOrganization.default_savings.map(
          (saving) => ({
            name: saving.name,
