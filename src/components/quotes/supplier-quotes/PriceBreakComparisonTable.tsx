...
const getUnitCostForProduct = (
  priceBreak: SupplierQuotePriceBreak,
  productIndex: number
): number | null => {
  const columnName =
    `unit_cost_${productIndex}` as keyof SupplierQuotePriceBreak;
...
export function PriceBreakComparisonTable({
  ...
  const filteredQuotes = useMemo(() => {
    return quotes.filter((quote) => {
      if (!includeExpiredQuotes) {
        const isExpired = quote.valid_to
          ? new Date(quote.valid_to) < new Date()
          : false;
  ...
  const { comparisonData, supplierQuoteGroups, globalMaxProducts } =
    useMemo(() => {
      ...
      filteredQuotes.forEach((quote) => {
        ...
        quote.price_breaks.forEach((priceBreak) => {
          ...
          if (!dataMap.has(quantity)) {
            ...
            existingData.maxProducts = Math.max(
              existingData.maxProducts,
              numProducts
            );
          }
      ...
      const finalData = {
        comparisonData: Array.from(dataMap.values()).sort(
          (a, b) => a.quantity - b.quantity
        ),
        supplierQuoteGroups: Array.from(supplierGroups.values()).sort((a, b) =>
    ...
    return (
      <div className="text-center p-8 border border-dashed rounded-md">
        <p className="text-muted-foreground">
          No price break data available for comparison.
        </p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse border border-gray-300">
          <thead>
            ...
            <tr>
              <th
                className="border border-gray-300 p-3 bg-gray-50 font-semibold text-left"
                rowSpan={3}
              >
                Quantity
              </th>
              {Array.from({ length: globalMaxProducts }, (_, productIndex) => {
                const totalQuotesForProduct = supplierQuoteGroups.reduce(
                  (total, group) => total + group.quoteIds.length,
                  0
                );
                return (
                  <th
                        ...
                        <div className="space-y-1">
                          ...
                          <div>
                            {isDraft ? (
                              ...
                              <Badge
                                variant="outline"
                                className="bg-red-50 text-red-800 border-red-200 text-xs"
                              >
                                Expired
                              </Badge>
                            ) : (
                ...
                {Array.from({ length: globalMaxProducts }, (_, productIndex) =>
                  supplierQuoteGroups.map((group) =>
                    group.quoteIds.map((quoteId) => {
                      ...
                      const productCell =
                        quoteData?.productPrices[productIndex];
                      ...
                      const bestPrice = getBestPriceForProduct(
