...
export function SupplierQuoteDialog({ 
  ...
  const { useCreateSupplierQuote, useSubmitSupplierQuote, useUpdateSupplierQuote } = useSupplierQuotes();
  ...
  useEffect(() => {
    if (open && quoteRequest) {
      if (mode === 'edit' && existingQuote) {
        setCreatedQuoteId(existingQuote.id);
  ...
  const getInitialProductionSchedule = () => {
    ...
    } else if (quoteRequest.production_schedule_requested && 
    ...
    if (submissionType === 'draft') {
      if (mode === 'edit' && createdQuoteId) {
        ...
        updateMutation.mutate({
          ...
          onSuccess: () => {
            console.log('Successfully updated supplier quote with ID:', createdQuoteId);
  ...
  const getInitialValues = (): SupplierQuoteFormValues => {
    if (mode === 'edit' && existingQuote) {
      return {
        ...
        packaging_carton_quantity: existingQuote.packaging_carton_quantity || null,
        ...
        packaging_cartons_per_pallet: existingQuote.packaging_cartons_per_pallet || null,
        packaging_copies_per_20ft_palletized: existingQuote.packaging_copies_per_20ft_palletized || null,
        packaging_copies_per_40ft_palletized: existingQuote.packaging_copies_per_40ft_palletized || null,
        packaging_copies_per_20ft_unpalletized: existingQuote.packaging_copies_per_20ft_unpalletized || null,
        packaging_copies_per_40ft_unpalletized: existingQuote.packaging_copies_per_40ft_unpalletized || null,
      };
    }
  ...
  const dialogTitle = mode === 'edit' ? "Edit Quote Response" : "Create Quote Response";
