...
export function PricingTab({ control, quoteRequest, mode }: PricingTabProps) {
  ...
  useEffect(() => {
    if (!quoteRequest || !quoteRequest.formats || initializing === false)
      return;
    ...
    quoteRequest.formats.forEach((format) => {
      ...
      const sortedPriceBreaks = [...format.price_breaks].sort(
        (a, b) => a.quantity - b.quantity
      );

      sortedPriceBreaks.forEach((priceBreak: any) => {
        newPriceBreaks.push({
          id: "",
          supplier_quote_id: "",
          ...
          unit_cost: priceBreak?.unit_cost || null,
          unit_cost_1: priceBreak?.unit_cost_1 || null,
          unit_cost_2: priceBreak?.unit_cost_2 || null,
          unit_cost_3: priceBreak?.unit_cost_3 || null,
          unit_cost_4: priceBreak?.unit_cost_4 || null,
          unit_cost_5: priceBreak?.unit_cost_5 || null,
          unit_cost_6: priceBreak?.unit_cost_6 || null,
          unit_cost_7: priceBreak?.unit_cost_7 || null,
          unit_cost_8: priceBreak?.unit_cost_8 || null,
          unit_cost_9: priceBreak?.unit_cost_9 || null,
          unit_cost_10: priceBreak?.unit_cost_10 || null,
        });
      });
    });
  ...
  const getProductHeadings = (numProducts: number) => {
    return Array.from(
      { length: numProducts },
      (_, i) => `${i + 1} Title${i + 1 > 1 ? "s" : ""}`
    );
  };

  const formatIds =
    quoteRequest.formats?.map((format) => format.format_id).filter(Boolean) ||
    [];
  ...
  return (
    <div className="space-y-3">
      {Object.entries(priceBreaksByFormat).map(([formatId, priceBreaks]) => {
        ...
        const format = quoteRequest.formats?.find((f) => f.id === formatId);
        const numProducts = format?.price_breaks[0]?.num_products || 1;
        // const numProducts = format?.num_products || 1;
        ...
        const formatDescription =
          numProducts > 0
            ? `Please supply unit cost for each quantity break and ${numProducts} Title${
                numProducts > 1 ? "s" : ""
              }`
            : "No products for this format";

        const FormatSpecificationsWrapper = () => {
          const { data: formatDetails, isLoading } = useFormatDetails(
