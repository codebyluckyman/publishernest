...
export function ExtraCostItem({
  ...
  const { fields } = useFieldArray({
    control,
    name: `extra_costs.${index}.price_breaks` as const
  });

  // Watch the field values 
  const fieldValues = useWatch({
    control,
    name: `extra_costs.${index}.price_breaks`
  });

  const handleOpenChange = (open: boolean) => {
    onOpenChange(extraCost.id, open);
  };
    ...
    >
      <div className="flex flex-col h-full">
        <CollapsibleTrigger asChild>
          ...
          >
            <div>
              <h3 className="text-base font-medium">{extraCost.name}</h3>
                      ...
                      <div className="grid grid-cols-3 md:grid-cols-12 gap-2 items-center">
                        ...
                        <div className="col-span-2 md:col-span-10">
                          {showMultiProducts ? (
                              ...
                              {Array.from({ length: Math.min(maxNumProducts, 10) }, (_, i) => {
                                ...
                                return (
                                  <FormField
                                    key={`${priceBreak.id}-${i}`}
                                    control={control}
                                    name={`extra_costs.${index}.price_breaks.${fieldIndex}.${fieldName}` as any}
                                    render={({ field }) => (
                                      <FormItem>
                                        <FormControl>
                                          <Input
                            ...
                            <FormField
                              control={control}
                              name={`extra_costs.${index}.price_breaks.${fieldIndex}.unit_cost` as any}
                              render={({ field }) => (
                                <FormItem>
                                  <FormControl>
                                    <Input
