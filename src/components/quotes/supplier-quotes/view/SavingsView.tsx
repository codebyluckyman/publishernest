
import { SupplierQuote } from "@/types/supplierQuote";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { formatCurrency } from "@/utils/formatters";
import { useUnitOfMeasures } from "@/hooks/useUnitOfMeasures";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";

interface SavingsViewProps {
  quote: SupplierQuote;
}

export function SavingsView({ quote }: SavingsViewProps) {
  const { unitOfMeasures } = useUnitOfMeasures();

  // If no savings, show empty state
  if (!quote.quote_request?.savings || quote.quote_request.savings.length === 0) {
    return (
      <div className="text-center p-8 bg-muted rounded-lg">
        <h3 className="text-lg font-medium">No Savings</h3>
        <p className="text-muted-foreground mt-2">
          This quote request does not have any savings defined.
        </p>
      </div>
    );
  }

  // Group all savings by unit of measure
  const groupedSavings: Record<string, typeof quote.quote_request.savings> = 
    quote.quote_request.savings.reduce((acc, saving) => {
      // Find the unit of measure
      const unitOfMeasure = unitOfMeasures.find(
        (unit) => unit.id === saving.unit_of_measure_id
      );
      
      // Get unit name or default to 'Other'
      const unitName = unitOfMeasure?.name || 'Other';
      
      // Create group if it doesn't exist
      if (!acc[unitName]) {
        acc[unitName] = [];
      }
      
      // Add to group
      acc[unitName].push(saving);
      return acc;
    }, {} as Record<string, typeof quote.quote_request.savings>);

  // Helper function to get number of products for a format
  const getNumProductsForFormat = (formatId: string): number => {
    if (!quote.quote_request || !quote.quote_request.formats) return 1;
    
    const format = quote.quote_request.formats.find((f: any) => f.id === formatId);
    return format ? format.num_products || 1 : 1;
  };

  return (
    <div className="space-y-6">
      {/* Render all unit of measure groups */}
      {Object.entries(groupedSavings).map(([unitName, savings]) => {
        if (savings.length === 0) return null;
        
        // Check if this group contains inventory units
        const hasInventoryUnits = savings.some(saving => {
          const unitOfMeasure = unitOfMeasures.find(
            (unit) => unit.id === saving.unit_of_measure_id
          );
          return unitOfMeasure?.is_inventory_unit;
        });
        
        // If the group has inventory units, render product-specific savings
        if (hasInventoryUnits) {
          return (
            <Card key={unitName} className="mb-4">
              <CardHeader className="pb-3">
                <CardTitle className="text-base">{unitName} Savings</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6 p-0">
                {savings.map((saving) => {
                  const unitOfMeasure = unitOfMeasures.find(
                    (unit) => unit.id === saving.unit_of_measure_id
                  );
                  
                  // Only render inventory unit savings
                  if (!unitOfMeasure?.is_inventory_unit) return null;
                  
                  // Get all supplier savings for this saving ID
                  const supplierSaving = quote.savings?.find(
                    (s) => s.saving_id === saving.id
                  );
                  
                  if (!supplierSaving) return null;
                  
                  // Get number of products from the first format
                  const formatId = quote.quote_request?.formats?.[0]?.id;
                  const numProducts = getNumProductsForFormat(formatId || '');
                  
                  return (
                    <div key={saving.id} className="p-4 border-b">
                      <h3 className="font-medium">{saving.name}</h3>
                      <p className="text-sm text-muted-foreground mb-4">
                        {saving.description || 'No description'} ({unitOfMeasure?.name || 'Unknown unit'})
                      </p>
                      
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                        {Array.from({ length: numProducts }, (_, index) => {
                          const savingKey = `unit_cost_${index + 1}` as keyof typeof supplierSaving;
                          const savingValue = supplierSaving[savingKey];
                          
                          return (
                            <div key={index} className="bg-muted p-3 rounded">
                              <div className="text-xs text-muted-foreground mb-1">Product {index + 1}</div>
                              <div className="font-medium">
                                {savingValue !== null && savingValue !== undefined
                                  ? formatCurrency(Number(savingValue), quote.currency)
                                  : '-'}
                              </div>
                            </div>
                          );
                        })}
                      </div>
                    </div>
                  );
                })}
              </CardContent>
            </Card>
          );
        }
        
        // For non-inventory units, render regular table
        return (
          <Card key={unitName} className="mb-4">
            <CardHeader className="pb-3">
              <CardTitle className="text-base">{unitName} Savings</CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Name</TableHead>
                    <TableHead>Description</TableHead>
                    <TableHead>Saving</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {savings.filter(saving => {
                    // Only show non-inventory units in this table
                    const unit = unitOfMeasures.find(u => u.id === saving.unit_of_measure_id);
                    return !unit?.is_inventory_unit;
                  }).map((saving) => {
                    const supplierSaving = quote.savings?.find(
                      (s) => s.saving_id === saving.id
                    );
                    
                    return (
                      <TableRow key={saving.id}>
                        <TableCell>{saving.name}</TableCell>
                        <TableCell className="text-muted-foreground">
                          {saving.description || 'No description'}
                        </TableCell>
                        <TableCell>
                          {supplierSaving?.unit_cost !== null && 
                           supplierSaving?.unit_cost !== undefined ? (
                            <span className="font-medium">
                              {formatCurrency(supplierSaving.unit_cost, quote.currency)}
                            </span>
                          ) : (
                            <span className="text-muted-foreground">Not provided</span>
                          )}
                        </TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
              </Table>
            </CardContent>
          </Card>
        );
      })}
    </div>
  );
}
