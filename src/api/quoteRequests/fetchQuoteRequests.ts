...
export async function fetchQuoteRequests(
  params: FetchQuoteRequestsParams
  ...
  try {
    ...
    if (searchQuery && searchQuery.trim()) {
      ...
      query = query.or(
        `title.ilike.%${searchQuery.trim()}%,reference_id.ilike.%${searchQuery.trim()}%`
      );
    }

    if (users && users.trim()) {
      query = query.eq("requested_by", users.trim());
    }

    if (supplier && supplier.trim()) {
      query = query.eq("supplier_id", supplier.trim());
    }
    ...
    if (quoteRequests && quoteRequests.length > 0) {
      ...
      if (quoteRequests[0].formats && quoteRequests[0].formats.length > 0) {
        ...
        console.log(
          "Format name access:",
          quoteRequests[0].formats[0].format?.format_name
        );
      }
    ...
    const enrichedRequests = await Promise.all(
      (quoteRequests || []).map(async (request) => {
        ...
        const processedFormats =
          request.formats?.map((formatItem: any) => {
            return {
              ...formatItem,
              format_name: formatItem.format?.format_name || "Unknown Format",
              products:
                formatItem.products?.map((product: any) => ({
                  ...product,
        ...
        const formattedSavings =
          request.savings?.map((saving: any) => {
            return {
              id: saving.id,
              quote_request_id: request.id,
              name: saving.name,
        ...
        console.log(
          `Quote Request ${request.reference_id} - required_step:`,
          request.required_step
        );

        // Extract step name from required_step object
        const required_step_name = request.required_step
          ? request.required_step.step_name
          : null;

        console.log(
          `Quote Request ${request.reference_id} - required_step_name extracted:`,
          required_step_name
        );

        // Ensure the status is one of the valid enum values
        const validStatus = ["pending", "approved", "declined"].includes(
          request.status
        )
